...
第 4–8 页默认不放底部关键判断条，正文由框架图和必要说明组成。TikZ 使用 `y=1.4cm` 扩展纵向布局，节点文字不随坐标缩放；复杂图可以直接在腾出的空间内增加层级。第 14 页保留六色图例。
...
第 14 页提供六色复杂框架示例与完整图例，可复制为新的方法总览。每页按需使用颜色，不必用满六种。
...
```latex
\node[idea innovation,text width=28mm,minimum height=13mm]
