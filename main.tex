...
\begin{frame}{方法框架}
  \framesubtitle{蓝色表征、紫色交互、青绿输出；珊瑚色表示训练反馈}
  \begin{center}
  \begin{tikzpicture}[y=1.4cm]
    ...
    \node[idea solid,text width=21mm,minimum height=10mm] (b) at (7,-.15) {$h=g_\phi(z,c)$};
    ...
    \draw[idea arrow,draw=IdeaTeal] (b)--node[idea note,above]{$h$}(c);
...
\begin{frame}{条件感知的特征交互}
  ...
  \begin{tikzpicture}[y=1.4cm]
    ...
    \node[idea solid,text width=28mm] (g) at (7.1,0) {条件调制\\$v=\gamma(c)\odot u$};
    ...
    \draw[idea arrow] (p)--(g);
    \draw[idea arrow,draw=IdeaTeal] (g)--(h);
    \draw[idea arrow,draw=IdeaAmber] (cond)--(g);
...
\begin{frame}{用颜色组织复杂框架}
  \framesubtitle{同一职责保持同色；模块名称与线型共同解释信息流}
  \begin{center}
  \begin{tikzpicture}[y=1.35cm]
    ...
    \node[idea solid,text width=26mm,minimum height=13mm] (core) at (4.1,-.55) {核心交互模块\\$h=g(z_1,z_2,c)$};
  ...
  \begin{center}
  \begin{tikzpicture}
    \foreach \xx/\tone/\label in {0/IdeaBlue/数据与表征,2.65/IdeaPurple/核心交互,5.3/IdeaAmber/条件与约束,7.95/IdeaTeal/任务输出,10.6/IdeaCoral/验证与反馈}{
